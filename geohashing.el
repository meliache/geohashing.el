...
and returns the Dow Jones Industrial Average as a string, if known for that date.
In case that DJIA for that date is unkown, fails, as no error handling
is implemented yet. Uses API at carabiner.peeron.com. "
...
Returns the element of list for which fn-to-min gets minimal."
  (second (reduce (lambda (x y) (if (< (car x) (car y)) x y))
